...
echo.
echo +============================================================+
echo !       ACTUALIZADOR - RTMyG Administracion                  !
echo +============================================================+
echo.

:: ---------------------------------------------------------------
:: PASO 1: Verificar que el proceso no este corriendo
:: ---------------------------------------------------------------
echo [1/5] Verificando que el programa no este en ejecucion...

tasklist /FI "IMAGENAME eq %NOMBRE_EXE%" 2>nul | find /I "%NOMBRE_EXE%" >nul
if not errorlevel 1 (
    echo.
    echo +============================================================+
    echo !  ERROR: El programa esta en ejecucion.                     !
    echo !  Por favor cierralo antes de actualizar.                   !
    echo +============================================================+
    echo.
    pause
    exit /b 1
)
echo       OK - Programa no esta en ejecucion
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ProgressPreference = 'SilentlyContinue'; $url = '%VERSIONES_DOC_URL%'; try { Invoke-WebRequest -Uri $url -UseBasicParsing -OutFile '%TEMP_VERSIONES%'; } catch { Write-Host 'ERROR_DESCARGA'; }"
...
if %TXT_SIZE% LSS 10 (
    echo.
    echo +============================================================+
    echo !  ERROR: Lista de versiones vacia o inaccesible.            !
    echo +============================================================+
    echo.
...
echo       OK - Lista de versiones obtenida

:: ---------------------------------------------------------------
:: PASO 3: Mostrar ultimas 10 versiones y permitir seleccion
:: ---------------------------------------------------------------
echo [3/5] Procesando versiones disponibles...
echo.

:: Usar PowerShell para procesar el archivo y mostrar menu
powershell -NoProfile -ExecutionPolicy Bypass -Command "$content = Get-Content '%TEMP_VERSIONES%' -Encoding UTF8; $versiones = @(); $currentVer = $null; $currentHora = $null; foreach ($line in $content) { $trimmed = $line.Trim(); if ($trimmed -match '^(\d+)\s+(\d{3,4})\s+(https://.+)$') { $hora = $matches[2].PadLeft(4,'0'); $horaFmt = $hora.Substring(0,2) + ':' + $hora.Substring(2,2); $versiones += [PSCustomObject]@{Ver=$matches[1]; Hora=$horaFmt; Url=$matches[3]}; } elseif ($trimmed -match '^(\d+)\s+(https://.+)$') { $versiones += [PSCustomObject]@{Ver=$matches[1]; Hora=$null; Url=$matches[2]}; } elseif ($trimmed -match '^(\d+)\s+(\d{3,4})$') { $currentVer = $matches[1]; $hora = $matches[2].PadLeft(4,'0'); $currentHora = $hora.Substring(0,2) + ':' + $hora.Substring(2,2); } elseif ($trimmed -match '^(\d+)$') { $currentVer = $matches[1]; $currentHora = $null; } elseif ($trimmed -match '^https://' -and $currentVer) { $versiones += [PSCustomObject]@{Ver=$currentVer; Hora=$currentHora; Url=$trimmed}; $currentVer = $null; $currentHora = $null; } } $total = $versiones.Count; if ($total -eq 0) { Write-Host 'NO_VERSIONES'; exit 1; } $ultimas = if ($total -le 10) { $versiones } else { $versiones[($total-10)..($total-1)] }; $ultimas = $ultimas | Sort-Object { [int]$_.Ver }, { $_.Hora } -Descending; $i = 1; Write-Host '+============================================================+'; Write-Host '!         VERSIONES DISPONIBLES (ultimas 10)                 !'; Write-Host '+------------------------------------------------------------+'; foreach ($v in $ultimas) { if ($v.Hora) { $verText = '{0} ({1})' -f $v.Ver, $v.Hora } else { $verText = $v.Ver }; $display = '  [{0,2}] Version {1}' -f $i, $verText; Write-Host ('!' + $display.PadRight(60) + '!'); Add-Content '%TEMP_MENU%' ('{0}|{1}|{2}' -f $i, $verText, $v.Url); $i++; } Write-Host '+------------------------------------------------------------+'; Write-Host '!  [99] Actualizar este instalador                           !'; Write-Host '+============================================================+';"

echo.

:seleccionar_version
set /p "SELECCION=Selecciona una opcion: "
...
:: Verificar si es opcion 99 (actualizar instalador)
if "%SELECCION%"=="99" goto actualizar_instalador

:: Buscar la seleccion en el archivo temporal
set "VERSION_ELEGIDA="
set "URL_DESCARGA="
...
echo.
echo       OK - Seleccionada: Version %VERSION_ELEGIDA%
...
if exist "%NOMBRE_EXE%" (
    ...
    if not exist "%CARPETA_BACKUP%\!BACKUP_BASE!" (
        ...
        echo       OK - Backup creado: %CARPETA_BACKUP%\!BACKUP_BASE!
    ) else (
        set "CONTADOR=1"
        :buscar_nombre
        ...
        echo       OK - Backup creado: %CARPETA_BACKUP%\!BACKUP_NOMBRE!
    )
) else (
...
:: ---------------------------------------------------------------
echo [5/5] Descargando version %VERSION_ELEGIDA% desde el servidor...

set "TEMP_FILE=%TEMP%\nuevo_admin_%RANDOM%.exe"

:: Usar PowerShell para descargar
powershell -NoProfile -ExecutionPolicy Bypass -Command "$ProgressPreference = 'SilentlyContinue'; $url = '%URL_DESCARGA%'; $output = '%TEMP_FILE%'; try { Invoke-WebRequest -Uri $url -UseBasicParsing -OutFile $output; if ((Test-Path $output) -and ((Get-Item $output).Length -gt 1000)) { Write-Host 'DESCARGA_OK'; } else { Write-Host 'DESCARGA_FAIL'; } } catch { Write-Host 'DESCARGA_FAIL'; Write-Host $_.Exception.Message; }"
...
:: Verificar tamano minimo (evitar archivos HTML de error)
for %%A in ("%TEMP_FILE%") do set "FILE_SIZE=%%~zA"
if %FILE_SIZE% LSS 10000 (
    echo.
    echo +============================================================+
    echo !  ERROR: El archivo descargado parece invalido.             !
    echo !  El archivo solicitado no esta disponible.                 !
    echo +============================================================+
    echo.
    del "%TEMP_FILE%" 2>nul
    del "%TEMP_VERSIONES%" 2>nul
...
echo       OK - Descarga completada (%FILE_SIZE% bytes)
...
if exist "%NOMBRE_EXE%" (
    echo       OK - Ejecutable reemplazado exitosamente
    echo.
    echo +============================================================+
    echo !                                                            !
    echo !   ACTUALIZACION A VERSION %VERSION_ELEGIDA% COMPLETADA              !
    echo !                                                            !
    echo +============================================================+
) else (
    echo.
...
:: ---------------------------------------------------------------
:: ACTUALIZAR INSTALADOR (Opcion 99)
:: ---------------------------------------------------------------
...
echo +============================================================+
echo.
echo Descargando nueva version del instalador...

powershell -NoProfile -ExecutionPolicy Bypass -Command "$ProgressPreference = 'SilentlyContinue'; try { $content = (Invoke-WebRequest -Uri '%UPDATER_URL%' -UseBasicParsing -TimeoutSec 30).Content; [System.IO.File]::WriteAllText('%TEMP_UPDATER%', $content, [System.Text.Encoding]::ASCII); Write-Host 'OK'; } catch { Write-Host 'ERROR'; Write-Host $_.Exception.Message; }" > "%TEMP%\updater_result.txt"
...
if "%UPDATER_RESULT%"=="OK" (
    ...
    :: Crear script de actualizacion diferida
